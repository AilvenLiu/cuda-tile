...
cuda_tile.module @module {
  cuda_tile.testing$func @make_strided_view_wrong_tensor_view_elem(%tensor_view: !cuda_tile.tensor_view<4096x4096xf64, strides=[4096,1]>) {
    // expected-note @above{{prior use here}}
    // expected-error @below{{expects different type than prior uses}}
    cuda_tile.make_strided_view %tensor_view : !cuda_tile.strided_view<tile=(1024x1024), traversal_strides=[1024,1], !cuda_tile.tensor_view<4096x4096xf32, strides=[4096,1]>>
  }
}
...
cuda_tile.module @kernels {
  cuda_tile.testing$func @load_missing_index(%memref_i8: !cuda_tile.tensor_view<1024xi8, strides=[1]>) {
    %view_i8 = make_strided_view %memref_i8 : strided_view<tile=(128), traversal_strides=[1], tensor_view<1024xi8, strides=[1]>>
    // expected-error @below{{expected 1 index operands (based on view type), got 0}}
...
cuda_tile.module @module {
  cuda_tile.testing$func @store_view_tko_index_type_scalar_pv(%view: !cuda_tile.partition_view<tile=(1024x1024), !cuda_tile.tensor_view<4096x4096xf32, strides=[4096,1]>>, %tile: !cuda_tile.tile<1024x1024xf32>) {
...
cuda_tile.module @module {
  cuda_tile.testing$func @store_view_tko_index_type_scalar_sv(%view: !cuda_tile.strided_view<tile=(1024x1024), traversal_strides=[1024,1], !cuda_tile.tensor_view<4096x4096xf32, strides=[4096,1]>>, %tile: !cuda_tile.tile<1024x1024xf32>) {
    %c0_i32 = cuda_tile.constant <i32: 0> : !cuda_tile.tile<1xi32>
    %c0_i64 = cuda_tile.constant <i32: 0> : !cuda_tile.tile<1xi32>
    // expected-error @+1{{expected index type to be a scalar tile, got '!cuda_tile.tile<1xi32>'}}
